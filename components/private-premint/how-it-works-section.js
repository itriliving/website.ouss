...
export default function HowItWorksSection() {
	...
	const stepsObj = t('howItWorks.steps', { returnObjects: true }) || {};

	const steps = Object.keys(stepsObj)
		.sort((a, b) => parseInt(a) - parseInt(b))
		.map((key) => {
			const step = stepsObj[key];
			if (step.bullets && typeof step.bullets === 'object') {
				return {
					...step,
					bullets: Object.keys(step.bullets)
						.sort((a, b) => parseInt(a) - parseInt(b))
						.map((bulletKey) => step.bullets[bulletKey]),
				};
			}
			return step;
		});
